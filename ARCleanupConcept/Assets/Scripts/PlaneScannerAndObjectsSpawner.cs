...
public class PlaneScannerAndObjectsSpawner : MonoBehaviour
{
    ...
    IEnumerator TryFillSpawnsUntilMax(ARPlane hintPlane)
    {
        ...
        while (spawnedObjects.Count < target && spawnedObjs < target && safeLoopGuard < maxTotalAttempts)
        {
            safeLoopGuard++;
    ...
    public void NotifyItemRemoved()
    {
        ...
        // Start a refill attempt using TryFillSpawnsUntilMax. We can call with null hint plane,
        // TryFillSpawnsUntilMax is a coroutine already defined in the spawner script.
        StartCoroutine(TryFillSpawnsUntilMax(null));
    }
    public void ForceClearAllSpawned()
    {
        // destroy spawned objects if any
        ...
        // clear placed positions
        if (placedPositions != null) placedPositions.Clear();
        ...
        // reset flags so spawn will happen again
        hasSpawned = false;
    }

    /// <summary>
    /// Reset internal state used to detect area increases and spawning.
    /// Call after ARSession.Reset() so future plane updates will trigger spawning again.
    /// </summary>
    public void ResetSpawnerState()
    {
        // ensure internal lists in sync
        CleanSpawnedList();

        // reset area tracking and spawn flag
        previousTotalArea = 0f;
        hasSpawned = false;
    }
